    ...
    for track in args.tracks:
        ...
        for episode in range(args.eval_episodes):
            ...
            while not done:
                ...
                action, agent_state = agent.action(obs=obs['A'], reset=np.array([done]), state=agent_state, log=True)
                action = {'A': np.array(action)}
                obs, rewards, dones, info = env.step(action)
                done = dones['A']
            agent.write_activity(outdir=basedir / "neural_activity", episode_id=episode)
...
def parse():
    ...
    parser.add_argument('--checkpoint_dir', type=pathlib.Path, required=True)
    ...
    parser.add_argument('--eval_episodes', nargs='?', type=int, default=1)
